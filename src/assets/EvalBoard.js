...
BoardCLASS.prototype.Eval = function (c) {
    ...
    for (let j = 0; j < 8; j++) {
        ...
        if (rock[0][j] && !n1) r += 20;
        if (rock[1][j] && !n2) r -= 20;
        if (rock[0][j] && !n1 && !n2) r += 10;
        if (rock[1][j] && !n1 && !n2) r -= 10;
    }
    if (state) {
        const [w,b] = this.coordKings
        if (c === 1 && r < 100) r +=  kingCenter[w] + 100 - parseInt(Distance(w,b) * 10 )
        if (c === 0 && r > 100) r -=  kingCenter[b] + 100 - parseInt(Distance(w,b) * 10 )
    }
    if (c) return -r;
    return r;
}